...
If `$PAPERCLIP_BASE_URL` is unset or empty at the start of your heartbeat, refuse to proceed and PATCH your subtask `blocked` with `BLOCKED: $PAPERCLIP_BASE_URL not injected — operator must set it in this agent's adapter config.` Do not guess a default — the operator owns the host. `$PAPERCLIP_BASE_URL` is distinct from `$GITHUB_TOKEN` (used for `gh` / git auth against GitHub) — they are unrelated and both must be set.
